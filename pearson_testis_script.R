...
for (i in 1:nrow(testis_data)) {# Loop through each brain isoform and compute correlation
  ...
  cor_test <- cor.test(rfx_isoform_1, testis_isoform,method = "pearson") #perform pearson corrilation
  ...
  isoform_1_results[i, "pearson"] <- cor_test$estimate
}

# Write the Excel file
write_xlsx(isoform_1_results, "pearson_testis_RFX1.xlsx")
